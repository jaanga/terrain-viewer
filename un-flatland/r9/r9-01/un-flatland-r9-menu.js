...
//		console.log( this, this.value, this.selectedIndex );
	...
	function updatePlacards() {
		...
		msg.innerHTML = 'pointHome ' + uf.lat + ' ' + uf.lon + b +
			'pointHome tiles: ' + pointHome.tileX + ' ' + pointHome.tileY + b +
			'Tile ul Lat: ' + pointHome.ulTileLat.toFixed(5) + ' lon:' + pointHome.ulTileLon.toFixed(5) + b +
			'Tile delta lat: ' + pointHome.deltaLat.toFixed(5) + ' lon:' + pointHome.deltaLon.toFixed(5) + b + b +
			'Point x: ' + pointHome.ptX.toFixed(5) + ' y:' + pointHome.ptY.toFixed(5) + b + b +
		'';
		...
		for ( var i = 0, iLen = uf.gazetteer.length; i < iLen; i++ ) {
			...
			if ( place[1] < uf.ulLat && place[1] > uf.lrLat && place[2] > uf.ulLon  && place[2] < uf.lrLon ) {
				...
				var offsetX = -256 + uf.tileWidth * ( point.tileX - pointHome.tileX );
				var offsetY = -256 + uf.tileWidth * ( point.tileY - pointHome.tileY );

				var mesh = drawObject( point.ptX + offsetX, 10, point.ptY + offsetY);
				mesh.scale.set( 10, 10, 10 );
				uf.placards.add( mesh );

				mesh = drawSprite( place[0], '#0f0', point.ptX + offsetX, 120, point.ptY + offsetY);
				uf.placards.add( mesh );
			}
		}
...
// moving things about - bottom menu commands
	function getTile( direction ) {
		...
		uf.lon = tile2lon( point.tileX, uf.zoom);
		uf.lat = tile2lat( point.tileY, uf.zoom);
